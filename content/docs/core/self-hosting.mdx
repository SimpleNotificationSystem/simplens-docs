...
<Tabs items={['Linux', 'Windows', 'npm']}>
  <Tab value="Linux">
    ...
    <Callout type="info">
    Works on Ubuntu, Debian, CentOS, and other Linux distros. Requires `curl` and `bash`.
    </Callout>
  </Tab>
  ...
  <Tab value="Windows">
    ...
    <Callout type="warn">
    Run PowerShell as Administrator. Requires PowerShell 5.1 or later.
    </Callout>
  </Tab>
...
2. Generate secure API keys and secrets
3. Create a starter plugin configuration
4. Start the infrastructure and application services
