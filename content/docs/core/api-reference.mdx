...
<div className="flex gap-2 mb-4">
  <LLMCopyButton markdownUrl="/docs/core/api-reference.mdx" />
  <ViewOptions 
    markdownUrl="/docs/core/api-reference.mdx"
    githubUrl="https://github.com/SimpleNotificationSystem/simplens-docs/blob/main/content/docs/core/api-reference.mdx"
  />
</div>
...
<TypeTable
  type={{
    ...
    template_id: {
      description: 'Array of template IDs, position-mapped to channels. Use null for channels using inline content.',
      type: 'string[]'
    },
    content: {
      description: 'Key-value map of channel content. Required if template_id is not provided.',
      type: 'Record<string, unknown>',
    },
    variables: {
...
<Callout type="info">
Either `template_id` or `content` must be present. You can also mix both — see [Notification Templates](./notification-templates#using-templates-in-notification-requests) for details.
</Callout>
...
<TypeTable
  type={{
    ...
    content: {
      description: 'Content shared across all recipients. Required if template_id is not provided.',
      type: 'Record<string, unknown>',
    },
    recipients: {
