...
SimpleNS is split into two logical halves to make deployment flexible.

| Concept | Docker Compose File | Services Included | Purpose |
|---------|---------------------|-------------------|---------|
| **Application** | `docker-compose.yaml` | `api`, `worker`, `processor`, `recovery`, `dashboard` | The Node.js services that make up the SimpleNS logic. These are stateless (except for recovery) and can be scaled horizontally. |
| **Infrastructure** | `docker-compose.infra.yaml`| `mongodb`, `redis`, `kafka`, `zookeeper`, `loki`, `grafana` | The stateful databases and event buses. You can self-host these or use managed cloud equivalents (e.g., MongoDB Atlas, AWS MSK, ElastiCache). |

<Callout type="info" title="All-in-one Deployment">
For local development or simple single-machine deployments, `docker-compose.dev.yaml` combines both Application and Infrastructure into one file.
</Callout>
...
#### Provider Fallback (Quick Rule)

1. Try the default (or explicit) provider.
2. If the error is **non-retryable**, validate against fallback schema.
3. If valid, try fallback provider immediately.
4. If both fail, mark as `failed`.

<Callout type="info">
Retryable errors (for example rate limit or timeout) do not use immediate fallback first. They follow retry/backoff and may hand off to fallback only after retry budget is exhausted.
</Callout>
...
#### Fallback Routing Modes

SimpleNS applies two separate fallback mechanisms:

<Mermaid
  chart="
flowchart TD
    A[Provider Attempt] --> B{Error Type?}
    B -->|Non-retryable| C[Immediate Fallback<br/>sendWithFallback]
    B -->|Retryable or Rate-Limited| D{retry_count + 1 > MAX_RETRY_COUNT?}
    D -->|No| E[Backoff + Delayed Retry<br/>Same Provider]
    D -->|Yes| F[Retry-Budget Handoff<br/>scheduleFallbackProviderHandoff]
...
#### How Fallback Provider Is Resolved

Fallback resolution is guarded to avoid circular routing:

<Mermaid
  chart="
flowchart TD
    A[Resolve Fallback for Channel] --> B{channelConfig.fallback set?}
    B -->|Yes| C[Use Configured Fallback]
    B -->|No| D[Use 2nd Registered Provider]
    C --> E{fallback == currentProvider?}
    D --> E
    E -->|Yes| F[No Handoff / No Immediate Fallback]
    E -->|No| G[Use Distinct Fallback Provider]

    style F fill:#ef4444,stroke:#dc2626,color:#fff
    style G fill:#22c55e,stroke:#16a34a,color:#fff
"/>

#### Delayed Re-Entry After Handoff

Handoff is asynchronous and re-enters through the delayed pipeline:

<Mermaid
  chart="
flowchart LR
    A[Unified Consumer<br/>publishDelayed] --> B[delayed_notification Topic]
    B --> C[Delayed Consumer]
    C --> D[Redis Delayed Queue]
    D --> E[Delayed Poller Claims Due Event]
    E --> F[Target Producer]
    F --> G[channel_notification Topic]
    G --> H[Unified Consumer Reprocesses<br/>provider=fallback, retry_count=0]
...
When handoff is scheduled successfully:

- Unified consumer returns success for the current Kafka message.
- Offset is committed.
- Continuation is represented by the new delayed event.

If handoff cannot be scheduled, terminal failure handling is used.
